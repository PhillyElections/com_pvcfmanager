...
/**
 * Cycle Model for PVCFManager Component
 *
 * @package    Philadelphia.Votes
 * @subpackage Components
 * @license    GNU/GPL
 */
class PvcfmanagerModelCycle extends JModel
{
    /**
    ...
    public function &getData()
    {
        // Load the data
        if (empty($this->_data)) {
            $query = ' SELECT * FROM `#__pv_cf_cycles` ' . ' WHERE `id` = ' . $this->_db->quote($this->_id);
    ...
    public function store()
    {
        ...
        // Bind the form fields to the Cycle table
        if (!$row->bind($data)) {
            $this->setError($this->_db->getErrorMsg());
            return false;
        }

        // Make sure the Cycle record is valid
        if (!$row->check()) {
            //$this->setError($this->_db->getErrorMsg());
